...
// GET all tutorials (public) with ratings
router.get('/', async (req, res) => {
  try {
    console.log('GET /api/tutorials');
    ...
    // Add ratings to each tutorial
    const tutorialsWithRatings = await Promise.all(tutorials.map(async (tutorial) => {
      const ratingData = await Rating.getAverageRating(tutorial._id);
      return {
        ...tutorial.toObject(),
    ...
    console.log(`Found ${tutorialsWithRatings.length} tutorials`);
    res.json(tutorialsWithRatings);
  } catch (error) {
    console.error('Error fetching tutorials:', error);
    res.status(500).json({ error: 'Failed to fetch tutorials' });
  }
...
// GET single tutorial by ID with ratings and comments
router.get('/:id', async (req, res) => {
  try {
    const tutorial = await Tutorial.findById(req.params.id)
      .populate('createdBy', 'email fullName');
    ...
    // Get ratings and comments for this tutorial
    const ratings = await Rating.find({ tutorial: tutorial._id })
...
router.post('/:id/rate', authRequired, async (req, res) => {
  try {
    ...
    const ratingDoc = await Rating.findOneAndUpdate(
      { tutorial: tutorialId, user: req.user.id },
      { rating, comment },
      { upsert: true, new: true, runValidators: true }
    );
    
    // Get updated average rating
    const ratingData = await Rating.getAverageRating(tutorialId);
...
router.post('/:id/bookmark', authRequired, async (req, res) => {
  try {
    ...
    const user = await req.user.bookmarkTutorial(tutorialId);
    
    res.json({ message: 'Tutorial bookmarked successfully' });
  } catch (error) {
    console.error('Error bookmarking tutorial:', error);
    res.status(500).json({ error: 'Failed to bookmark tutorial' });
  }
});

// Remove bookmark from tutorial
router.delete('/:id/bookmark', authRequired, async (req, res) => {
  try {
    ...
    const user = await req.user.unbookmarkTutorial(tutorialId);
